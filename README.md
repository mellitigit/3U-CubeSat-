...
This CubeSat system combines five AI-driven objectives to enable fully autonomous operation. It predicts downlink capacity and applies adaptive compression to maximize data transmission using the communication subsystem. It forecasts power availability to optimize payload scheduling and prevent brownouts, leveraging the electrical power subsystem. The battery health module monitors temperature, voltage, and current to prevent overheating, supported by thermal sensors and MOSFET switches. The AI-based FDIR subsystem detects, isolates, and recovers from anomalies across sensors and subsystems to maintain reliability. Finally, the detumbling and attitude stabilization module uses reaction wheels, magnetorquers, and reinforcement learning to reduce angular velocity efficiently. Together, these objectives integrate AI models with hardware and mechanical subsystems to ensure autonomous, reliable CubeSat operation without ground intervention.
